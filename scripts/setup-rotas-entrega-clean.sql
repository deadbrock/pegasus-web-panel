-- ============================================================================
-- VERSAO CLEAN: Rotas de Entrega
-- Remove tudo antes e recria do zero
-- ============================================================================
...
DROP FUNCTION IF EXISTS criar_rota_automatica();
DROP FUNCTION IF EXISTS update_rotas_updated_at();

-- PASSO 2: Remover tabela antiga
DROP TABLE IF EXISTS rotas_entrega CASCADE;

-- PASSO 3: Remover sequence antiga
DROP SEQUENCE IF EXISTS seq_rotas;

-- ============================================================================
-- PASSO 4: CRIAR TABELA rotas_entrega
-- ============================================================================
...
CREATE INDEX idx_rotas_pedido_id ON rotas_entrega(pedido_id);
CREATE INDEX idx_rotas_motorista_id ON rotas_entrega(motorista_id);
CREATE INDEX idx_rotas_veiculo_id ON rotas_entrega(veiculo_id);
CREATE INDEX idx_rotas_status ON rotas_entrega(status);
CREATE INDEX idx_rotas_data_prevista ON rotas_entrega(data_prevista_entrega);
CREATE INDEX idx_rotas_created_at ON rotas_entrega(created_at DESC);
...
-- Política: Admins e logística podem gerenciar
...
-- ============================================================================
-- PASSO 8: SEQUENCE para números de rotas
-- ============================================================================

CREATE SEQUENCE seq_rotas START 1;

-- ============================================================================
-- PASSO 9: TRIGGER criar rota automaticamente
-- ============================================================================
...
BEGIN
  ...
  IF NEW.status = 'Em Separação' AND (OLD.status IS NULL OR OLD.status != 'Em Separação') THEN
    ...
    RAISE NOTICE 'Rota % criada para pedido %', v_numero_rota, NEW.numero_pedido;
  END IF;
...
BEGIN
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Rotas: %', v_count;
  RAISE NOTICE 'Trigger: Ativo';
  RAISE NOTICE 'RLS: Habilitado';
  RAISE NOTICE '';
  RAISE NOTICE 'FLUXO:';
  RAISE NOTICE '1. Pedido status -> Em Separação';
  RAISE NOTICE '2. Rota criada automaticamente';
  RAISE NOTICE '3. Status: Aguardando Atribuição';
  RAISE NOTICE '4. Logística atribui motorista/veículo';
  RAISE NOTICE '5. Motorista inicia entrega';
  RAISE NOTICE '========================================';
END $$;
